...
import { CredentialItem, photonGetConfig } from "../networking"
...
// Interface for photon config response
...
const ProviderSpecificFields: React.FC<ProviderSpecificFieldsProps> = ({
  ...
  const [photonConfig, setPhotonConfig] = useState<PhotonConfig | null>(null)

  // Fetch Photon config when component mounts
  useEffect(() => {
    console.log("useEffect for photon config triggered:", {
      hasAccessToken: !!accessToken,
      accessTokenStart: accessToken?.substring(0, 10) + "...",
    })

    const fetchPhotonConfig = async () => {
      try {
        if (!accessToken) {
          console.log("No access token provided for photon config fetch")
          return
        }

        console.log("Fetching photon config using photonGetConfig...")
        const config = await photonGetConfig(accessToken)
        setPhotonConfig(config)
        console.log("Photon config loaded successfully:", config)
      } catch (error) {
        console.log("Error fetching photon config:", error)
      }
    }
  ...
  // Auto-complete API Base for Photon providers when provider changes or config loads
  useEffect(() => {
    if (!photonConfig) return

    if (selectedProviderEnum === Providers.PhotonOpenAI) {
      form.setFieldsValue({ api_base: photonConfig.inference_base + "/v1" })
    }

    if (selectedProviderEnum === Providers.PhotonAnthropic) {
      form.setFieldsValue({ api_base: photonConfig.inference_base })
    }
  }, [selectedProviderEnum, photonConfig, form])
